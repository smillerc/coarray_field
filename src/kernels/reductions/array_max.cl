...
__kernel void find_max(__global const float *input, __global float *output,
  ...
  for (int i = local_size / 2; i > 0; i /= 2) {
    if (local_id < i) {
      local_cache[local_id] =
          max(local_cache[local_id + i], local_cache[local_id]);
    }
  ...
  if (!local_id) {
    ...
    while (new_val != 0.0) {
      ...
      new_val = atomic_xchg(
          &output[0],
          max(old_val,
              new_val)); // change output[0] for the maximum of the
